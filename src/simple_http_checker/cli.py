import logging
import click


from .checker import check_urls

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S"
)

logger = logging.getLogger(__name__)


@click.command()
@click.argument("urls", nargs=-1)
@click.option("--timeout", default=5, help="Maximum time to wait for a response from each URL.")
@click.option("--verbose", "-v",is_flag=True, help="Enable verbose output.")
def main(urls, timeout, verbose):
    if verbose:
        logging.getLogger().setLevel(logging.DEBUG)
        logger.debug("Verbose mode enabled.")
    
    logger.debug(f"Received {urls}")
    logger.debug(f"Received timeout: {timeout}")
    logger.debug(f"Received verbose: {verbose}")

    if not urls:
        logger.warning("No URLs provided to check.")
        click.echo("Please provide at least one URL to check.")
        return
    
    logger.info(f"Starting URL check for {len(urls)}")

    results = check_urls(urls, timeout)

    click.echo("\nURL Check Results:")  
    for url, status in results.items():
        if "OK" in status:
            click.echo(click.style(f"{url:<40} {status}", fg="green"))
        else:
            click.echo(click.style(f"{url:<40} {status}", fg="red"))